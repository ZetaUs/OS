...
:: Create a proper hard disk image with logo data
:: Layout:
::   LBA 0: boot sector (512 bytes)
::   LBA 1-64: stage2 (32KB)
::   LBA 65-114: logo.raw (25600 bytes = 50 sectors)
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$boot=[System.IO.File]::ReadAllBytes('%OUT%\boot.bin'); $stage2=[System.IO.File]::ReadAllBytes('%OUT%\stage2.bin'); $logo=[System.IO.File]::ReadAllBytes('%~dp0logo.raw'); $imgSize=16*63*100*512; $pad=New-Object byte[] ($imgSize-512-32768-25600); $img=New-Object byte[] $imgSize; $boot.CopyTo($img,0); $stage2.CopyTo($img,512); $logo.CopyTo($img,512+32768); $pad.CopyTo($img,512+32768+25600); [System.IO.File]::WriteAllBytes('%OUT%\nova-os.img',$img)"
